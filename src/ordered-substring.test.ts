...
test("conatins two, no rearrangement", () => {
  expect(inArray(["xyz", "live", "strong"], ["lively", "alive", "harp", "sharp", "armstrong"]).toStrictEqual(expect.arrayContaining(["live", "strong"])));
});
test("contains three, with rearrangement", () => {
    expect(inArray(["live", "strong", "arp"], ["lively", "alive", "harp", "sharp", "armstrong"]).toBe(["arp", "live", "strong"]));
  });
